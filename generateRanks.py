...
def rankWithClassifier(cur_node, cur_neighbors_list, embeddings_model, classifier_model, vector_feat):
    # Create the instances:
    X_test = []
    cur_node_embed = embeddings_model.getEmbeddingsForWord(cur_node)
    for cur_n in cur_neighbors_list:
    ...
    new_neighbors_list = [None] * len(new_ranks)
    for i in range(len(new_ranks)):
        new_neighbors_list[new_ranks[i]-1] = (cur_neighbors_list[i], y_pred_probs_pos[i])
...
def computeRanks_with_classifier(lexicon_evaluation, embeddings_file_path, embeddings_dims, classifier_model_path):
    ...
    for cur_line in gpt_fobj:
        ...
        try:
            word_neighbors_all = sorted(embeddings_model.getVocabWords())
            word_neighbors_with_probs = rankWithClassifier(cur_query, word_neighbors_all, embeddings_model, classifier_model, "vector_diff")
...
def computeRanks(lexicon_evaluation, embeddings_file_path, embeddings_dims):
    ...
    for cur_line in gpt_fobj:
        ...
        try:
            word_neighbors_with_probs = embeddings_model.getMostSimilarWords(cur_query, -1, probs=True)
...
if __name__ == "__main__":
    ...
    if classifier_model_path is not None:
        ...
        rank_list = computeRanks(lexicon_evaluation, embeddings_file_path, embeddings_dims)
